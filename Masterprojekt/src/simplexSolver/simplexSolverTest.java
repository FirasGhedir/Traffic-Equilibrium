...
public class simplexSolverTest {
	...
	public static void test() {
		...
		// x + 3y = 6
		float[][] standardized = { { 1, 1, 1, 0, 4 }, { 1, 3, 0, 1, 6 }, { -3, -5, 0, 0, 0 } };

		// row and column do not include
		// right hand side values
		// and objective row
		SimplexSolver simplex = new SimplexSolver(2, 4);
		...
		while (!quit) {
			SimplexSolver.ERROR err = simplex.compute();

			if (err == SimplexSolver.ERROR.IS_OPTIMAL) {
				simplex.print();
				quit = true;
			} else if (err == SimplexSolver.ERROR.UNBOUNDED) {
